...
class BeerController(private val repository: BeerRepository, private val breweryRepository: BreweryRepository) {
    ...
    @PostMapping
    private fun createBeer(@RequestBody beer: BeerDto): BeerDto {
        ...
        return BeerDto(created.name, created.type, created.brewery.id!!, created.alcohol, created.description, created.reviews, created.stars)
    }
    ...
    @GetMapping("/{beerId}")
    private fun beer(@PathVariable beerId: Long): BeerDto {
...
internal class BeerDto(val name: String, val type: String, val breweryId: Long, val alcohol: Float, val description: String, val reviews: Int = 0, val stars: Float = 0f)